...
- **Data flow**: `poller.poll()` → `store.SetUsage/SetError` → `store` broadcasts a cap-1 channel → `/ws` handler → `view.Build(snaps)` JSON → browser (mustache OOB swap) **and** TUI. `internal/view` is the shared wire model; `internal/format` does reset-time formatting.
...
- **`index.html` is served as raw `go:embed` bytes, not via Go's template engine** — mustache `{{ }}` collides with Go template syntax.
